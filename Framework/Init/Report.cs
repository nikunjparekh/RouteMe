...
namespace Framework.Init
{
    public class Report
    {
        ...
        {
            if (sbHtml == null) sbHtml = new StringBuilder();
            sbHtml.Append("<div class='passed'> " + strLine + "</div><br>");
        ...
        {
            if (sbHtml == null) sbHtml = new StringBuilder();
            sbHtml.Append("<div class='orange'> " + strLine + "</div><br>");
      ...
      /*  public static void AddToHtmlReportFailed(IWebDriver driver, Exception ex, string strLine, bool IsItalic = false)
        {
            try
            {
                IWebElement Login_lbl_ServerError = driver.FindElement(By.XPath(ElementLocators.Login_lbl_ServerError));
                Report.AddToHtmlReportOrange("'Server is unable to display the page you requested.'");
                driver.Quit();
            }
            catch (Exception)
            {
                try
                {
                    IWebElement Login_lbl_AccessDenied = driver.FindElement(By.XPath(ElementLocators.Login_lbl_AccessDenied));
                    Report.AddToHtmlReportOrange("'Access Denied  !'");
                    driver.Quit();
                }
                catch (Exception)
                {
                    if (!ex.Message.ToString().Contains("Unexpected error"))
                    {
                        if (sbHtml == null) sbHtml = new StringBuilder();
                        sbHtml.Append("<table cellpadding='2' cellspacing='0'><tr><td style='width:5%;white-space: nowrap;' >");
                        sbHtml.Append("<div class='failed'> " + strLine + "</div>");
                        sbHtml.Append("</td><td>");
                        sbHtml.Append("<div class='dvSnap'> [<a href='" + GetScreenShot(driver) + "' target='_blank'> <b>View Snap</b> </a>] </div>");
        ...
        public static void AddToHtmlReportFeatureFinish()
        {
            try
            {
                ...
                /* *************** Commented to remove pass/failed images from group header *************************************
                if (IsPassed == 1)
                {
                ...
                else if (IsPassed == 3)
                {
                    ...
                    {
                        Console.WriteLine("22222222222222");
                        sbHtml.Replace("<div id='TC" + (TCcnt - 1) + "' class='passed'", "<div id='TC" + (TCcnt - 1) + "' class='orange'");
                    }
                    else if (sbHtml.ToString().Contains("<div id='TC" + (TCcnt - 1) + "' class='orange'"))
                    {
                        //Console.WriteLine("333333333333333");
                        //sbHtml.Replace("<div id='TC" + (TCcnt - 1) + "' class='passed'", "<div id='TC" + (TCcnt - 1) + "' class='failed'");
                    }
                    else sbHtml.Insert(((sbHtml.ToString().IndexOf("::"))), "<div id='TC" + (TCcnt - 1) + "' class='orange' style='float:left; height:16px; margin-top:2px;' ></div>");
                    Console.WriteLine("66666666666666");
